    ...
    <body class="hold-transition sidebar-mini layout-fixed">
        <div class="wrapper">
            ...
            <div class="content-wrapper">
                <?php 
                    $contentHeaderOptions   =   ['pageName' => 'FAQ'];
                                    ...
                                    <div class="card-header">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-8 text-right">
                                                <a href="<?=site_url(adminControllers('faq/add'))?>">
                                                    <button class="btn btn-link btn-sm">FAQ Baru</button>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card-body table-responsive">
                                        <table class="table table-sm" id='tabelFAQ'>
                                            <thead>
                                                <th class='border-top-0 text-center text-bold'>No.</th>
                                                <th class='border-top-0 text-left text-bold'>FAQ</th>
...
<script language='Javascript'>
    ...
    let _dataTableOptions   =  {
        ...
        serverSide  :   true,
        ajax    :   {
            url         :   `<?=base_url(adminControllers('faq/listFAQ'))?>${location.search}`,
            dataSrc     :   'listFAQ'
        },
        ...
        columns     :   [
            ...
            {data : null, render : function(data, type, row, metaData){
                let _idFAQ      =   data.id;
                let _titleFAUC  =   data.title.toUpperCase();

                return `<div class='text-center'>
                            <a href='${_siteURL}${_adminControllers}/faq/edit/${_idFAQ}'>
                                <span class='fa fa-edit text-warning cp mr-2' data-toggle='tooltip' data-placement='top'
                                    title='Edit Data FAQ ${_titleFAUC}'></span>
                            </a>
                            <span class='fa fa-trash text-danger cp' data-toggle='tooltip' data-placement='top'
                                title='Hapus Data FAQ ${_titleFAUC}' onClick='hapusFAQ(this, "${_idFAQ}", "${_titleFAUC}")'></span>
                        </div>`;
            }}
        ]
    };

    let _faqDataTable   =   $('#tabelFAQ').DataTable(_dataTableOptions);

    async function hapusFAQ(thisContext, idFAQ, judulFAQ) {
        let _el                 =   $(thisContext);

        let _title          =   'Konfirmasi Hapus';
        let _htmlMessage    =   `Apakah anda yakin akan menghapus FAQ <b class='text-primary'>${judulFAQ}</b> ?`;
        let _confirm        =   await confirmSwal(_title, _htmlMessage);
        
        if(_confirm){
            $.ajax({
                url     :   '<?=site_url(adminControllers('faq/process_delete'))?>',
                data    :   `idFAQ=${idFAQ}`,
                type    :   'POST',
                success :   (decodedRFS) => {
                    let _statusHapus    =   decodedRFS.statusHapus;
                    
                    let _title  =   'Hapus FAQ';
                    let _icon, _message, _onClick;
                    if(_statusHapus){
                        _icon       =   'success';
                        _message    =   `<span class='text-success'>Berhasil menghapus FAQ!</span>`;   
                        _onClick    =   function(){
                            _faqDataTable.draw();
                        };
                    }else{
